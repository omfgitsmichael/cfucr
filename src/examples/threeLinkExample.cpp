...
{ 
  ...
  robot::ThreeLinkRobot robot;
  
  // Initialize the controller with the desired classes, the config file, and the robot //
  robot::Controller<robot::ThreeLinkRobot, robot::ThreeLinkControl, robot::Filter<robot::ThreeLinkRobot>> controller(configFile1, robot);
  ...
  robot::Filter<robot::ThreeLinkRobot> filter = controller.filter();
